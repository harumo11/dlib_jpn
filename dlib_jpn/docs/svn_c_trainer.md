...
## classes
- svn_c_trainer
 ２値分類問題を解くためにC サポートベクターマシーンを学習させます．
 そして，その学習の結果として **decision_function** クラスを出力します．このクラスは，**SMO** アルゴリズムを用いて実装されています．
 C-SVN学習アルゴリズムの実装には以下の論文を参考にしました．
 >Chih-Chung Chang and Chih-Jen Lin, LIBSVM : a library for support vector machines, 2001. Software available at [libsvn](http://www.csie.ntu.edu.tw/~cjlin/libsvm)

```
// このコンテンツはパブリックドメインに属しています．詳しくはLICENSE_FOR_EXAMPLE_PROGRAMS.txtを見てください．
/*
	このサンプルはdlibを利用してサポートベクターマシーンの使い方を説明しています．
	特に，このサンプルのなかではsvmのCパラメータの設定の仕方を説明しています．

	このサンプルはsvmを学習させるための簡単なデータ集合を作成し，
	あなたに交差検証(cross validation)の仕方と，
	我々のデータセットを分類するための,
	優れた決定関数を見つけるためのsvm トレーニング関数の使い方を示します．

	このサンプルで使用されるデータは２次元で，原点から
	１０未満の距離の点に+1とラベル付けされており，その他の点は-1
	とラベル付されています．
